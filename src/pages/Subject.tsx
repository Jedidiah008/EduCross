    ...
    return (
      <div className="min-h-screen bg-gradient-hero flex items-center justify-center">
        <Card className="game-card p-8 text-center">
          <h2 className="text-2xl font-fredoka text-foreground mb-4">Subject Not Found</h2>
    ...
    <div className="min-h-screen bg-gradient-hero">
      ...
      <header className="border-b border-border bg-card/50 backdrop-blur-sm sticky top-0 z-50">
        <div className="container mx-auto px-4 py-4 flex items-center justify-between">
          ...
          <h1 className={`text-xl font-fredoka ${colorClass}`}>{subject.title}</h1>
        </div>
      </header>
      ...
      <main className="container mx-auto px-4 py-8">
        <div className="text-center mb-8 animate-slide-up">
          <h2 className={`text-3xl font-fredoka ${colorClass} mb-2`}>Choose a Unit</h2>
          <p className="text-muted-foreground">Select a unit to view its lessons</p>
        </div>
          ...
          {subject.units.map((unit, index) => (
            ...
            >
              <CardContent className="flex items-center gap-4 p-5">
                <div className={`w-14 h-14 rounded-xl ${bgColorClass} flex items-center justify-center shrink-0`}>
                  <BookOpen className={`w-7 h-7 ${colorClass}`} />
                </div>
                <div className="flex-1 min-w-0">
                  <h3 className="text-lg font-fredoka text-foreground truncate">{unit.title}</h3>
